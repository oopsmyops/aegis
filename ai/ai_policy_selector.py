...
class AIPolicySelector(AIPolicySelectorInterface):
    ...
    def __init__(
        ...
        policy_catalog_path: str = "./policy-catalog",
        output_directory: str = "./recommended-policies",
        ...
        enable_ai_fixes = (
            config.get("output", {}).get("fix_policies", False) if config else False
        )
        self.kyverno_validator = KyvernoValidator(
            bedrock_client, enable_ai_fixes=enable_ai_fixes
        )
        ...
        self.phase_one_candidates = self.two_phase_config.get(
            "phase_one_candidates", 150
        )
        self.phase_one_max_tokens = self.two_phase_config.get(
            "phase_one_max_tokens", 2000
        )
        self.phase_one_temperature = self.two_phase_config.get(
            "phase_one_temperature", 0.1
        )
        self.phase_two_max_tokens = self.two_phase_config.get(
            "phase_two_max_tokens", 4000
        )
        self.phase_two_temperature = self.two_phase_config.get(
    ...
    def select_policies_two_phase(
        ...
        try:
            self.logger.info(
                f"Starting Two-Phase AI policy selection for {target_count} policies"
            )
            ...
            for attempt in range(max_retries):
                try:
                    candidate_policy_names = self.phase_one_filter(
                        cluster_info, requirements, policy_index
                    )
                    self.logger.info(
                        f"Phase 1 completed: {len(candidate_policy_names)} candidates selected"
                    )
                    break
                except Exception as e:
                    self.logger.warning(
                        f"Phase 1 attempt {attempt + 1}/{max_retries} failed: {e}"
                    )
                    if attempt == max_retries - 1:
                        self.logger.warning(
                            "All Phase 1 attempts failed. Using fallback candidate selection."
                        )
                        candidate_policy_names = self._fallback_phase_one_selection(
                            policy_index
                        )
                    else:
            ...
            if not candidate_policy_names:
                self.logger.warning(
                    "No candidates from Phase 1, using fallback selection"
                )
                return self._fallback_policy_selection(
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    self.logger.info(
                        f"Phase 2 completed: {len(selected_policies)} policies selected"
                    )
                    break
                except Exception as e:
                    self.logger.warning(
                        f"Phase 2 attempt {attempt + 1}/{max_retries} failed: {e}"
                    )
                    if attempt == max_retries - 1:
                        self.logger.warning(
                            "All Phase 2 attempts failed. Using candidate policies directly."
                        )
                        selected_policies = self._map_policies_from_index(
                            candidate_policy_names[:target_count], policy_index
                        )
                    else:
                        # Wait before retry (exponential backoff)
                        import time
            ...
            if len(selected_policies) < target_count * 0.5:  # At least 50% of target
                self.logger.warning(
                    f"Insufficient policies selected ({len(selected_policies)}), supplementing with fallback"
                )
            ...
            self.logger.info(
                f"Two-Phase selection completed: {len(final_policies)} policies selected"
            )
    ...
    def select_policies(
        ...
        try:
            ...
            if two_phase_enabled:
                ...
                return self.select_policies_two_phase(
                    cluster_info, requirements, policy_index, target_count
                )
            else:
                self.logger.info(
                    "Two-Phase selection disabled, using legacy single-phase approach"
                )
                return self._legacy_single_phase_selection(
                    cluster_info, requirements, policy_index, target_count
                )
    ...
    def determine_categories(
        self,
        cluster_info: ClusterInfo,
        selected_policies: List[PolicyCatalogEntry],
        requirements: GovernanceRequirements,
    ) -> List[str]:
        """Dynamically determine output categories using AI."""
        return self.category_determiner.determine_categories(
    ...
    def validate_and_fix_policies(
        self, policies: List[RecommendedPolicy], output_dir: Optional[str] = None
    ) -> Tuple[List[RecommendedPolicy], List[Any]]:
        """
        ...
        try:
            ...
            for policy in policies:
                temp_validation_results.append(
                    type(
                        "ValidationResult",
                        (),
                        {
            ...
            created_files = self.output_manager.organize_policies_by_categories(
                type(
                    ...
                    (),
                    {
                        ...
                        "categories": [
                            p.category or p.original_policy.category for p in policies
                        ],
                        "cluster_info": None,
                        "requirements": None,
            ...
            validation_results, report_file = (
                self.kyverno_validator.validate_policies_with_report(
                    policies, output_dir
                )
            )
            ...
            for policy in policies:
                ...
                validation_result = next(
                    (
                        ...
                        if vr.policy_name == policy.original_policy.name
                    ),
                    None,
                )
                ...
                if validation_result:
                    ...
                    if validation_result.warnings:
                        policy.customizations_applied.extend(
                            [f"Warning: {w}" for w in validation_result.warnings[:2]]
                        )
                else:
            ...
            self.logger.info(
                f"Validation completed: {len(validation_results)} policies processed"
            )
    ...
    def generate_complete_recommendation(
        ...
        try:
            ...
            selected_policies = self.select_policies_two_phase(
                cluster_info, requirements, policy_index, target_count
            )
            ...
            customized_policies = self.customize_policies(
                selected_policies, requirements
            )
    ...
    def phase_one_filter(
        ...
        try:
            ...
            lightweight_policies = self._extract_lightweight_policies_from_index(
                policy_index
            )

            self.logger.info(
                f"Phase 1: Processing {len(lightweight_policies)} policies with lightweight metadata"
            )

            # Prepare context for Phase 1 filtering
            context = self._prepare_phase_one_context(
                cluster_info, requirements, lightweight_policies
            )
            ...
            self.logger.info(
                f"Phase 1: Filtered to {len(candidate_policy_names)} candidate policies"
            )
    ...
    def phase_two_select(
        ...
        try:
            self.logger.info(
                f"Starting Phase 2 detailed policy selection from {len(candidate_policy_names)} candidates"
            )
            ...
            detailed_policies = catalog_manager.get_policies_detailed(
                candidate_policy_names
            )

            self.logger.info(
                f"Phase 2: Retrieved detailed information for {len(detailed_policies)} candidate policies"
            )

            # Prepare context for Phase 2 selection
            context = self._prepare_phase_two_context(
                cluster_info, requirements, detailed_policies
            )
            ...
            phase_two_max_tokens = self.two_phase_config.get(
                "phase_two_max_tokens", 4000
            )
            phase_two_temperature = self.two_phase_config.get(
                "phase_two_temperature", 0.1
            )
            ...
            selected_policies = self._map_detailed_policies_to_entries(
                final_policy_names, detailed_policies
            )

            # Apply comprehensive policy customization
            customized_policies = self._apply_comprehensive_customization(
                selected_policies, requirements
            )

            self.logger.info(
                f"Phase 2: Selected and customized {len(customized_policies)} final policies"
            )
            ...
            return self._map_policies_from_index(
                candidate_policy_names[:target_count], policy_index
            )
    ...
    def generate_organized_output(
        ...
        try:
            ...
            if validation_results:
                validation_report = (
                    validation_results[0].validation_report.get("report_file")
                    if validation_results
                    else None
                )
                if not validation_report:
                    validation_report = self.output_manager.generate_validation_report(
                        validation_results
                    )
    ...
    def _prepare_selection_context(
        ...
        sampled_policies = self._sample_policies_for_ai(
            policy_index, max_policies=max_policies
        )
...
"""
    ...
    def _parse_selection_response(self, response: str) -> List[str]:
        ...
        try:
            ...
            raise AISelectionError(
                f"Failed to parse policy selection from AI response: {e}"
            )

    def _map_policies_from_index(
        self, policy_names: List[str], policy_index: PolicyIndex
    ) -> List[PolicyCatalogEntry]:
    ...
    def _sample_policies_for_ai(
        self, policy_index: PolicyIndex, max_policies: int = 50
    ) -> List[Dict[str, Any]]:
    ...
    def _supplement_with_fallback(
        ...
        for policy in fallback_policies:
            if (
                policy.name not in current_names
                and len(selected_policies) < target_count
            ):
    ...
    def _fallback_policy_selection(
        ...
        if any(
            answer.question_id == "img_registry_enforcement" and answer.answer
            for answer in requirements.answers
        ):
            essential_policies.extend(
                ["restrict-image-registries", "advanced-restrict-image-registries"]
            )

        if any(
            answer.question_id == "img_latest_tag_prevention" and answer.answer
            for answer in requirements.answers
        ):
            essential_policies.append("disallow-latest-tag")

        if any(
            answer.question_id == "img_vulnerability_scanning" and answer.answer
            for answer in requirements.answers
        ):
        ...
        if any(
            answer.question_id == "res_limits_required" and answer.answer
            for answer in requirements.answers
        ):
            essential_policies.append("require-pod-requests-limits")

        if any(
            answer.question_id == "res_requests_required" and answer.answer
            for answer in requirements.answers
        ):
            essential_policies.append("require-pod-requests-limits")
        ...
        if any(
            answer.question_id == "sec_non_root_required" and answer.answer
            for answer in requirements.answers
        ):
            essential_policies.extend(
                ["require-non-root-groups", "require-run-as-nonroot"]
            )

        if any(
            answer.question_id == "sec_privileged_prevention" and answer.answer
            for answer in requirements.answers
        ):
            essential_policies.extend(["deny-privileged-profile", "prevent-cr8escape"])

        if any(
            answer.question_id == "sec_capabilities_restriction" and answer.answer
            for answer in requirements.answers
        ):
            essential_policies.extend(
                ["require-drop-all", "restrict-adding-capabilities"]
            )

        if any(
            answer.question_id == "sec_readonly_filesystem" and answer.answer
            for answer in requirements.answers
        ):
        ...
        if any(
            answer.question_id == "net_policies_required" and answer.answer
            for answer in requirements.answers
        ):
            essential_policies.append("require-netpol")

        if any(
            answer.question_id == "net_ingress_restrictions" and answer.answer
            for answer in requirements.answers
        ):
            essential_policies.extend(
                ["require-ingress-https", "restrict-ingress-wildcard"]
            )

        # Compliance requirements
        if any(
            answer.question_id == "comp_labeling_standards" and answer.answer
            for answer in requirements.answers
        ):
            essential_policies.extend(["require-labels", "require-annotations"])

        if any(
            answer.question_id == "comp_pod_disruption_budgets" and answer.answer
            for answer in requirements.answers
        ):
        ...
        if len(selected_policies) < target_count:
            ...
            for category in priority_categories:
                if category in policy_index.categories and remaining_needed > 0:
                    ...
                    for policy in category_policies:
                        if (
                            not any(p.name == policy.name for p in selected_policies)
    ...
    def _read_test_content(self, policy: PolicyCatalogEntry) -> Optional[str]:
        ...
        try:
            if os.path.isdir(test_path):
                ...
                test_files = [
                    f for f in os.listdir(test_path) if f.endswith((".yaml", ".yml"))
                ]
    ...
    def _prepare_phase_one_context(
        self,
        cluster_info: ClusterInfo,
        requirements: GovernanceRequirements,
        lightweight_policies: List[Dict[str, Any]],
    ) -> Dict[str, Any]:
        ...
        context = {
            ...
            "policies_by_category": self._group_policies_by_category(
...
"""
    ...
    def _group_policies_by_category(
        self, lightweight_policies: List[Dict[str, Any]]
        ...
        try:
            ...
            if response.startswith("[") and response.endswith("]"):
                ...
                if json_match:
                    ...
                    self.logger.warning(
                        "Could not find valid JSON array, extracting policy names from text"
                    )
                    ...
                    if len(policy_names) < 20:
                        ...
                        for line in lines:
                            ...
                            if (
                                ...
                                and not line.startswith("{")
                                and not line.startswith("}")
                                and not line.lower().startswith("based on")
                                and not line.lower().startswith("here")
                                and not "```" in line
                                and not line.lower().startswith("json")
                                and not line.lower().startswith("the ")
                                and not line.lower().startswith("i ")
            ...
            for name in policy_names:
                if isinstance(name, str) and len(name.strip()) > 0:
                    ...
                    if (
                        ...
                        and not clean_name.lower().startswith("based on")
                    ):
                        cleaned_names.append(clean_name)

            self.logger.info(
                f"Parsed {len(cleaned_names)} policy names from Phase 1 response"
            )
    ...
    def _extract_lightweight_policies_from_index(
                ...
                lightweight_policies.append(
                    {
                        ...
                        "tags": policy.tags[
                            :5
                        ],  # Limit tags for lightweight processing
                    }
                )
    ...
    def _fallback_phase_one_selection(self, policy_index: PolicyIndex) -> List[str]:
        ...
        policies_per_category = max(
            1, target_candidates // len(policy_index.categories)
        )
        ...
        self.logger.info(
            f"Fallback selection: {len(final_candidates)} total candidates"
        )
    ...
    def _prepare_phase_two_context(
        self,
        cluster_info: ClusterInfo,
        requirements: GovernanceRequirements,
        detailed_policies: List[Dict[str, Any]],
    ) -> Dict[str, Any]:
...
"""
    ...
    def _map_detailed_policies_to_entries(
        self, policy_names: List[str], detailed_policies: List[Dict[str, Any]]
    ) -> List[PolicyCatalogEntry]:
        """Map selected policy names back to PolicyCatalogEntry objects."""
        selected_entries = []
    ...
    def _apply_comprehensive_customization(
        self, policies: List[PolicyCatalogEntry], requirements: GovernanceRequirements
    ) -> List[PolicyCatalogEntry]:
        """Apply comprehensive policy customization based on cluster requirements."""
        try:
            self.logger.info(
                f"Applying comprehensive customization to {len(policies)} policies"
            )
            ...
            for policy in policies:
                ...
                if requirements.registries:
                    customized_policy = self._apply_registry_customization(
                        customized_policy, requirements.registries
                    )

                # Apply label customizations
                if requirements.custom_labels:
                    customized_policy = self._apply_label_customization(
                        customized_policy, requirements.custom_labels
                    )

                # Apply compliance-specific customizations
                if requirements.compliance_frameworks:
                    customized_policy = self._apply_compliance_customization(
                        customized_policy, requirements.compliance_frameworks
                    )

                # Add AI-suggested customizations if available
                if (
                    hasattr(self, "_phase_two_customizations")
                    and policy.name in self._phase_two_customizations
                ):
            ...
            self.logger.info(
                f"Successfully applied customizations to {len(customized_policies)} policies"
            )
            return customized_policies
    ...
    def _apply_registry_customization(
        self, policy: PolicyCatalogEntry, registries: List[str]
    ) -> PolicyCatalogEntry:
        ...
        if (
            "registry" in policy.description.lower()
            or "image" in policy.description.lower()
        ):
            policy.description += (
                f" [Customized for registries: {', '.join(registries)}]"
            )

        return policy

    def _apply_label_customization(
        self, policy: PolicyCatalogEntry, custom_labels: Dict[str, str]
    ) -> PolicyCatalogEntry:
    ...
    def _apply_compliance_customization(
        self, policy: PolicyCatalogEntry, frameworks: List[str]
    ) -> PolicyCatalogEntry:
    ...
    def _apply_ai_suggested_customizations(
        self, policy: PolicyCatalogEntry, customizations: List[Dict[str, Any]]
    ) -> PolicyCatalogEntry:
    ...
    def _legacy_single_phase_selection(
        self,
        cluster_info: ClusterInfo,
        requirements: GovernanceRequirements,
        policy_index: PolicyIndex,
        ...
        try:
            ...
            selected_policies = self._map_policies_from_index(
                selected_names, policy_index
            )

            self.logger.info(
                f"Legacy single-phase selection completed: {len(selected_policies)} policies selected"
            )
            return selected_policies[:target_count]
    ...
    def _emergency_policy_selection(
        self, policy_index: PolicyIndex, target_count: int
    ) -> List[PolicyCatalogEntry]:
        """Emergency policy selection as last resort."""
        self.logger.warning(
            "Using emergency policy selection - basic sampling from all categories"
        )
    ...
    def _validate_two_phase_config(self) -> bool:
        ...
        try:
            ...
            if self.phase_one_candidates <= 0 or self.phase_one_candidates > 1000:
                self.logger.warning(
                    f"Invalid phase_one_candidates: {self.phase_one_candidates}"
                )
                return False

            if self.phase_one_max_tokens <= 0 or self.phase_one_max_tokens > 100000:
                self.logger.warning(
                    f"Invalid phase_one_max_tokens: {self.phase_one_max_tokens}"
                )
                return False
