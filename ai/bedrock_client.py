...
class BedrockClient(BedrockClientInterface):
    ...
    def send_request(
        ...
        if max_tokens <= 0 or max_tokens > 100000:
            raise AISelectionError(
                f"Invalid max_tokens: {max_tokens}. Must be between 1 and 100000"
            )
        ...
        for attempt in range(retry_count):
            try:
                self.logger.info(
                    f"Sending request to Bedrock model: {self.model_id} (attempt {attempt + 1}/{retry_count})"
                )
                ...
                if "claude" in self.model_id.lower():
                    if "content" in response_body and response_body["content"]:
                        ...
                        raise AISelectionError(
                            "Invalid response format from Claude model"
                        )
                elif "nova" in self.model_id.lower():
                    # Amazon Nova model response format
                    if (
                        "output" in response_body
                        and "message" in response_body["output"]
                    ):
                        ...
                        if "content" in message and message["content"]:
                            ...
                            raise AISelectionError(
                                "Invalid response format from Nova model"
                            )
                    else:
                        raise AISelectionError(
                            "Invalid response format from Nova model"
                ...
                self.logger.info(
                    f"Successfully received response from Bedrock (length: {len(response_text)})"
                )
                ...
                if error_code == "ValidationException":
                    ...
                    if attempt < retry_count - 1:
                        ...
                        raise NetworkError(
                            f"Request throttled after {retry_count} attempts: {error_message}"
                        )
                elif error_code == "ServiceUnavailableException":
                    if attempt < retry_count - 1:
                        wait_time = (2**attempt) + 1
                        self.logger.warning(
                            f"Service unavailable, retrying in {wait_time} seconds..."
                        )
                        time.sleep(wait_time)
                        continue
                    else:
                        raise NetworkError(
                            f"Service unavailable after {retry_count} attempts: {error_message}"
                        )
                else:
                    raise AISelectionError(
                ...
                if attempt < retry_count - 1:
                    wait_time = (2**attempt) + 1
                    self.logger.warning(
                        f"Network error, retrying in {wait_time} seconds: {e}"
                    )
                    time.sleep(wait_time)
                    continue
                else:
                    raise NetworkError(
                        f"Network error after {retry_count} attempts: {e}"
                    )
            except json.JSONDecodeError as e:
                if attempt < retry_count - 1:
                    wait_time = (2**attempt) + 1
                    self.logger.warning(
                        f"JSON decode error, retrying in {wait_time} seconds: {e}"
                    )
                    time.sleep(wait_time)
                    continue
                else:
                    raise AISelectionError(
                        f"Failed to parse Bedrock response after {retry_count} attempts: {e}"
                    )
            except Exception as e:
                if attempt < retry_count - 1:
                    wait_time = (2**attempt) + 1
                    self.logger.warning(
                        f"Unexpected error, retrying in {wait_time} seconds: {e}"
                    )
                    time.sleep(wait_time)
                    continue
                else:
                    raise AISelectionError(
                        f"Unexpected error after {retry_count} attempts: {e}"
                    )
    ...
    def send_request_with_fallback(
        ...
        try:
            ...
            self.logger.warning(
                f"Primary model {self.model_id} failed: {primary_error}"
            )
    ...
    def get_optimal_token_limit(self, prompt_length: int) -> int:
        ...
        available_tokens = (
            max_model_tokens - estimated_prompt_tokens - 1000
        )  # Safety buffer

        return min(available_tokens, response_tokens)

    def chunk_large_request(
        self, prompt: str, max_chunk_size: int = 50000
    ) -> List[str]:
