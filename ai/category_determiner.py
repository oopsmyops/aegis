    ...
    def determine_categories(
        ...
        try:
            ...
            validated_categories = self._validate_categories(
                categories, selected_policies
            )

            self.logger.info(
                f"Determined {len(validated_categories)} categories: {validated_categories}"
            )
...
"""
    ...
    def _validate_categories(
        self, categories: List[str], policies: List[PolicyCatalogEntry]
    ) -> List[str]:
        """Validate and filter categories."""
        if not categories:
    ...
    def _get_fallback_categories(self, policies: List[PolicyCatalogEntry]) -> List[str]:
        ...
        for policy in policies:
            ...
            if policy.category:
                category_counts[policy.category] = (
                    category_counts.get(policy.category, 0) + 1
                )
        ...
        if any(
            tag in ["aws", "gcp", "azure", "eks", "aks", "gke"] for tag in tag_counts
        ):
            fallback_categories.append("Platform Specific")

        return fallback_categories[:6]

    def assign_policies_to_categories(
        self, policies: List[PolicyCatalogEntry], categories: List[str]
    ) -> Dict[str, List[PolicyCatalogEntry]]:
        ...
        try:
            ...
            assignments = self._parse_assignment_response(
                response, policies, categories
            )
...
"""
    ...
    def _parse_assignment_response(
        self, response: str, policies: List[PolicyCatalogEntry], categories: List[str]
    ) -> Dict[str, List[PolicyCatalogEntry]]:
        ...
        try:
            ...
            unassigned_policies = [
                p for p in policies if p.name not in assigned_policies
            ]
            if unassigned_policies:
                # Distribute unassigned policies to categories with fewer policies
                sorted_categories = sorted(
                    categories, key=lambda c: len(category_mapping[c])
                )
    ...
    def _fallback_policy_assignment(
        self, policies: List[PolicyCatalogEntry], categories: List[str]
    ) -> Dict[str, List[PolicyCatalogEntry]]:
        ...
        for policy in policies:
            ...
            policy_text = (
                policy.name + " " + policy.description + " " + " ".join(policy.tags)
            ).lower()
            ...
            for category in categories:
                ...
                if not assigned:
                    if "security" in category_lower and any(
                        word in policy_text
                        for word in ["security", "rbac", "psp", "privileged"]
                    ):
                        category_mapping[category].append(policy)
                        assigned = True
                    elif "network" in category_lower and any(
                        word in policy_text
                        for word in ["network", "ingress", "egress", "service"]
                    ):
                        category_mapping[category].append(policy)
                        assigned = True
                    elif "resource" in category_lower and any(
                        word in policy_text
                        for word in ["resource", "limit", "quota", "memory", "cpu"]
                    ):
                        category_mapping[category].append(policy)
                        assigned = True
                    elif "platform" in category_lower and any(
                        word in policy_text
                        for word in ["aws", "gcp", "azure", "eks", "aks", "gke"]
                    ):
                        category_mapping[category].append(policy)
                        assigned = True
