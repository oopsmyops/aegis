...
class KyvernoValidator(PolicyValidatorInterface):
    ...
    def validate_policies_with_report(self, policies: List[RecommendedPolicy], 
                                    output_dir: str) -> Tuple[List[ValidationResult], str]:
        """
    ...
    def _validate_single_policy(self, policy: RecommendedPolicy, output_dir: str, 
                              cli_report: Dict[str, Any]) -> ValidationResult:
        ...
        if not has_existing_tests and self.enable_ai_fixes and self.test_case_generator:
            ...
            try:
                test_content = self.test_case_generator.generate_comprehensive_test_case(
                ...
                self.logger.error(f"Failed to generate test cases for {policy_name}: {e}")
        ...
        policy_cli_result = self._extract_policy_result_from_cli(policy_name, cli_report)
        ...
        if policy_cli_result:
            ...
            if self.enable_ai_fixes and cli_report.get('test_errors'):
                result = self._fix_test_file_errors(result, policy, policy_dir, cli_report['test_errors'])
        else:
            # No policy-specific CLI results available, but check if this policy failed in the global JSON
            if cli_report.get('json_output') and isinstance(cli_report['json_output'], list):
                for failure in cli_report['json_output']:
                    if isinstance(failure, dict) and failure.get('POLICY') == policy_name:
                        result.passed = False
                        result.errors.append(f"Policy failure: {failure.get('REASON', 'Unknown failure')}")
                        break
    ...
    def _apply_ai_fixes(self, result: ValidationResult, policy: RecommendedPolicy, 
                       policy_dir: str) -> ValidationResult:
        ...
        try:
            ...
            error_analysis = self._analyze_validation_errors(result.errors, policy.customized_content)
            ...
            if os.path.exists(test_file):
                fixed_test = self._fix_test_case_with_ai(test_file, result.errors, policy.customized_content)
                if fixed_test:
                    with open(test_file, 'w', encoding='utf-8') as f:
                        f.write(fixed_test)
            ...
            resource_file = os.path.join(policy_dir, "resource.yaml")
            if not os.path.exists(resource_file) or "no such file" in str(result.errors).lower():
                resource_content = self._generate_test_resources_with_ai(policy, error_analysis)
                with open(resource_file, 'w', encoding='utf-8') as f:
                    f.write(resource_content)
            ...
            if fixes_applied:
                ...
                self.logger.info(f"Applied AI fixes for {result.policy_name}: {fixes_applied}")
    ...
    def _execute_kyverno_cli_test(self, output_dir: str) -> Dict[str, Any]:
        ...
        try:
            ...
            result = subprocess.run(
                ["kyverno", "test", output_dir, "--require-tests", "--fail-only", "-ojson"],
                capture_output=True,
                text=True,
            ...
            if json_match:
                ...
                try:
                    ...
                    self.logger.info(f"Successfully parsed Kyverno JSON output with {len(json_output)} failure(s)")
                ...
                if not json_output:
                    ...
                    if result.returncode != 0 and ('failed' in result.stderr.lower() or 'error' in result.stderr.lower()):
                        self.logger.info("Kyverno CLI indicated failures but no JSON found")
            ...
            if result.stdout and 'Test errors:' in result.stdout:
                ...
                for line in stdout_lines:
                    ...
                    if line.startswith('Path:'):
                        ...
                        # Start new error
                        current_error = {'path': line.replace('Path:', '').strip(), 'error': ''}
                        collecting_error = False
                    elif line.startswith('Error:') and current_error:
                        current_error['error'] = line.replace('Error:', '').strip()
                        collecting_error = True
                    elif collecting_error and current_error and line and not line.startswith('Error:') and not line.startswith('Path:'):
                        # Continue collecting multi-line error message
                        if current_error['error']:
                            current_error['error'] += ' ' + line
                        else:
                            current_error['error'] = line
                    elif line.startswith('Error:') and 'found' in line and 'errors after loading tests' in line:
            ...
            self.logger.info(f"Kyverno CLI completed with return code: {result.returncode}")
            if result.stderr:
                self.logger.info(f"Kyverno CLI stderr contains: {len(result.stderr)} characters")
                # Log Test Summary if found
                if 'Test Summary:' in result.stderr:
                    summary_lines = [line for line in result.stderr.split('\n') if 'Test Summary:' in line]
    ...
    def _generate_yaml_report(self, validation_results: List[ValidationResult], 
                            output_dir: str) -> str:
        ...
        if kyverno_json:
            if isinstance(kyverno_json, list):
                ...
                self.logger.info(f"Kyverno JSON is a dict with keys: {list(kyverno_json.keys())}")
                
                # Check if it's a parse error case
                if 'parse_error' in kyverno_json:
                    self.logger.warning(f"JSON parse error: {kyverno_json['parse_error']}")
                    # Try to extract from raw_output if available
                    if 'raw_output' in kyverno_json:
                        raw_output = kyverno_json['raw_output']
                        self.logger.info(f"Trying to parse raw output: {raw_output[:200]}...")
                        try:
                ...
                self.logger.warning(f"Kyverno JSON is unexpected type: {type(kyverno_json)}")
        else:
        ...
        for result in validation_results:
            if result.test_results:
                ...
                if cli_output and 'Test Summary:' in cli_output:
                    ...
                    match = re.search(r'Test Summary:\s*(\d+)\s+out\s+of\s+(\d+)\s+tests?\s+failed', cli_output)
                    if match:
                        failed_tests = int(match.group(1))
                        total_tests = int(match.group(2))
                        self.logger.info(f"Extracted from Test Summary: {failed_tests} failed out of {total_tests} total tests")
                        break
                    
                    # Also look for pattern like "Test Summary: 229 tests passed"
                    match = re.search(r'Test Summary:\s*(\d+)\s+tests?\s+passed', cli_output)
                    if match and total_tests == 0:
                        total_tests = int(match.group(1))
                        failed_tests = 0
                        self.logger.info(f"Extracted from Test Summary: {total_tests} tests passed, 0 failed")
                        break
                    ...
                    if match and total_tests == 0:
                        ...
                        if 'failed' in cli_output.lower():
                            ...
                            total_tests = max(num1, num2)
                            failed_tests = min(num1, num2)
                        self.logger.info(f"Extracted from Test Summary (generic): {failed_tests} failed out of {total_tests} total tests")
                        break
        ...
        if total_tests == 0:
            ...
            if test_file_errors and failed_tests == 0:
                ...
                total_tests = len(validation_results)  # Assume each policy should have tests
            else:
        ...
        try:
            ...
            self.logger.info(f"Report summary: {total_tests} total tests, {failed_tests} failed, {success_rate:.1f}% success rate")
            return report_file
    ...
    def _analyze_validation_errors(self, errors: List[str], policy_content: str) -> Dict[str, Any]:
        ...
        for error in errors:
            ...
            elif 'fail' in error_lower and ('want pass' in error_lower or 'got fail' in error_lower):
    ...
    def _fix_test_file_errors(self, result: ValidationResult, policy: RecommendedPolicy, 
                             policy_dir: str, test_errors: List[Dict[str, str]]) -> ValidationResult:
        ...
        try:
            ...
            fixed_content = self._fix_malformed_test_file_with_ai(
                test_file_path, policy_test_errors, policy.customized_content, original_content
            )
            ...
            if fixed_content and fixed_content != original_content:
                ...
                try:
                    ...
                    result.warnings.append(f"Fixed test file parsing errors: {[e.get('error', '') for e in policy_test_errors]}")
                    self.logger.info(f"Successfully fixed test file errors for {result.policy_name}")
                    ...
                    if os.path.exists(backup_path):
                        ...
                        os.remove(backup_path)
                    result.warnings.append(f"Test file fix failed: Invalid YAML generated")
            else:
            ...
            self.logger.error(f"Error fixing test file errors for {result.policy_name}: {e}")
            ...
            if os.path.exists(backup_path):
                try:
                    with open(backup_path, 'r', encoding='utf-8') as f:
                        backup_content = f.read()
                    with open(os.path.join(policy_dir, "kyverno-test.yaml"), 'w', encoding='utf-8') as f:
                        f.write(backup_content)
                    os.remove(backup_path)
    ...
    def _fix_malformed_test_file_with_ai(self, test_file_path: str, test_errors: List[Dict[str, str]], 
                                        policy_content: str, original_content: str) -> Optional[str]:
...
"""
    ...
    def _fix_test_case_with_ai(self, test_file: str, errors: List[str], 
                              policy_content: str) -> Optional[str]:
...
"""
            ...
            response = self.bedrock_client.send_request(prompt, max_tokens=3000, temperature=0.2)
    ...
    def _generate_test_resources_with_ai(self, policy: RecommendedPolicy, 
...
"""
            ...
            response = self.bedrock_client.send_request(prompt, max_tokens=2500, temperature=0.2)
    ...
    def _check_kyverno_cli(self) -> bool:
        ...
        try:
            ...
            if available:
                ...
                self.logger.warning("Kyverno CLI not available - validation will be limited")
            return available
        ...
        try:
            ...
            if cli_report.get('json_output'):
                ...
                if isinstance(json_output, list):
                    ...
                    report['failed_policies'] = list(set(
                        failure.get('POLICY', '') for failure in json_output 
                ...
                if stdout:
                    ...
                    for line in lines:
                        ...
                        if 'FAIL:' in line:
                            ...
                            if len(parts) >= 2:
                                ...
                                policy_name = policy_parts[0] if len(policy_parts) > 0 else 'unknown'
                                
                                failure = {
                                    'policy': policy_name,
                                    'rule': policy_parts[1] if len(policy_parts) > 1 else 'unknown',
                                    'resource': policy_parts[2] if len(policy_parts) > 2 else 'unknown',
                                    'reason': reason
                                }
                    ...
                    if failures:
                        ...
                        report['failed_policies'] = list(set(
                            failure.get('policy', '') for failure in failures
                        ))
            ...
            if 'Test Summary:' in combined_output:
                import re
                match = re.search(r'(\d+)\s+out\s+of\s+(\d+)\s+tests?\s+failed', combined_output)
                if match:
            ...
            if report['total_tests'] > 0:
                report['success_rate'] = ((report['total_tests'] - report['failed_tests']) / report['total_tests']) * 100
    ...
    def _find_test_files(self, directory: str) -> List[str]:
        ...
        for root, dirs, files in os.walk(directory):
            for file in files:
                if file.endswith(('.yaml', '.yml')) and ('test' in file.lower() or file == 'kyverno-test.yaml'):
    ...
    def _parse_kyverno_output(self, output: str, stderr: str) -> Dict[str, Any]:
        ...
        # Calculate success rate
        if report['total_tests'] > 0:
            report['success_rate'] = round(((report['total_tests'] - report['failed_tests']) / report['total_tests']) * 100, 1)
    ...
    def _parse_test_file_errors(self, stderr: str) -> List[Dict[str, str]]:
        ...
        for line in lines:
            line = line.strip()
            if 'Error:' in line and ('failed to load' in line or 'failed to parse' in line):
    ...
    def _find_policy_directory(self, policy_name: str, output_dir: str) -> Optional[str]:
    ...
    def _extract_policy_result_from_cli(self, policy_name: str, 
                                      cli_report: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if isinstance(json_output, dict) and 'parse_error' in json_output:
                return {
                    'passed': False,
                    'errors': [f"Failed to parse Kyverno output: {json_output['parse_error']}"],
                    'warnings': [],
            ...
            if isinstance(json_output, dict) and json_output.get('all_tests_passed'):
                ...
                test_results = json_output if isinstance(json_output, list) else [json_output]
        ...
        if stderr and policy_name in stderr:
            ...
            for line in stderr_lines:
                if policy_name in line and ("error" in line.lower() or "fail" in line.lower()):
    ...
    def _parse_yaml_test_results(self, yaml_data: Dict[str, Any], policy_name: str) -> Dict[str, Any]:
    ...
    def _generate_fix_recommendations(self, validation_results: List[ValidationResult]) -> List[str]:
        ...
        for error_type, count in common_errors.items():
            if error_type == "missing_files":
                recommendations.append(f"Generate missing test files for {count} policies")
            elif error_type == "test_failures":
                recommendations.append(f"Review and fix test expectations for {count} policies")
        ...
        if self.enable_ai_fixes:
            ...
            if fixed_count > 0:
                recommendations.append(f"AI fixes were applied to {fixed_count} policies")
        else:
            recommendations.append("Enable --fix flag to apply AI-powered fixes automatically")
    ...
    def _clean_result_for_report(self, result: ValidationResult) -> Dict[str, Any]:
        ...
        return {
            ...
            'validation_timestamp': result.validation_report.get('timestamp') if result.validation_report else None
        }
    ...
    def _categorize_error(self, error: str) -> str:
        ...
        elif "namespace" in error_lower and ("default" in error_lower or "context" in error_lower):
    ...
    def generate_test_case(self, policy_content: str) -> str:
        ...
        try:
            ...
            return self._generate_minimal_test_case(policy_name if 'policy_name' in locals() else "unknown")
