...
class PolicyCustomizer:
        ...
        try:
            ...
            if not policy_data:
                raise AISelectionError(
                    f"Failed to parse policy content for {policy.name}"
                )
            ...
            if requirements.registries:
                policy_data, registry_changes = self._customize_registries(
                    policy_data, requirements.registries
                )
                customizations_applied.extend(registry_changes)

            # Apply label customizations
            if requirements.custom_labels:
                policy_data, label_changes = self._customize_labels(
                    policy_data, requirements.custom_labels
                )
                customizations_applied.extend(label_changes)

            # Apply parameter customizations based on requirements
            policy_data, param_changes = self._customize_parameters(
                policy_data, requirements
            )
            ...
            self.logger.info(
                f"Applied {len(customizations_applied)} customizations to {policy.name}"
            )
    ...
    def customize_policies_batch(
        ...
        for policy in policies:
            ...
            recommended_policy = self.customize_policy(
    ...
    def _customize_registries(
        self, policy_data: Dict[str, Any], allowed_registries: List[str]
        ...
        try:
                    ...
                    for rule in spec["rules"]:
                        if isinstance(rule, dict) and "validate" in rule:
                            ...
                            if isinstance(validate, dict):
                                changes.extend(
                                    self._update_registry_in_validation(
                                        validate, allowed_registries
                                    )
                                )
                            ...
                            if isinstance(verify_images, list):
                                for verify_rule in verify_images:
                                    if (
                                        isinstance(verify_rule, dict)
                                        and "imageReferences" in verify_rule
                                    ):
                                        # Update image references to use allowed registries
                                        old_refs = verify_rule["imageReferences"]
                                        if allowed_registries:
                                            verify_rule["imageReferences"] = [
                                                f"{reg}/*" for reg in allowed_registries
                                            ]
                                            changes.append(
                                                f"Updated imageReferences from {old_refs} to allowed registries"
                                            )
            ...
            for pattern in registry_patterns:
                if re.search(pattern, policy_yaml_str) and allowed_registries:
                    ...
                    changes.append(
                        f"Replaced registry pattern {pattern} with {new_registry}"
                    )
    ...
    def _customize_labels(
        self, policy_data: Dict[str, Any], custom_labels: Dict[str, str]
    ) -> tuple[Dict[str, Any], List[str]]:
            ...
            for key, value in custom_labels.items():
                ...
                if old_value != value:
                    if old_value is None:
                        ...
                        changes.append(
                            f"Updated label {key} from {old_value} to {value}"
                        )
                            ...
                            if isinstance(generate, dict) and "data" in generate:
                                ...
                                if isinstance(data, dict) and "metadata" in data:
                                    ...
                                    for key, value in custom_labels.items():
                                        ...
                                        changes.append(
                                            f"Added label {key}={value} to generated resource"
                                        )
    ...
    def _customize_parameters(
        self, policy_data: Dict[str, Any], requirements: GovernanceRequirements
    ) -> tuple[Dict[str, Any], List[str]]:
        ...
        try:
            ...
            requirement_flags = {
                answer.question_id: answer.answer for answer in requirements.answers
            }
    ...
    def _update_registry_in_validation(
        self, validate: Dict[str, Any], allowed_registries: List[str]
    ) -> List[str]:
                    ...
                    if "containers" in spec:
                        for container in spec["containers"]:
                            if "image" in container and allowed_registries:
                                ...
                                changes.append(
                                    f"Updated image pattern from {old_image} to {new_pattern}"
                                )
    ...
    def _enable_resource_limit_enforcement(
                ...
                if "spec" in policy_data and "rules" in policy_data["spec"]:
                    for rule in policy_data["spec"]["rules"]:
                        if isinstance(rule, dict) and "validate" in rule:
                            ...
                            changes.append(
                                "Enabled stricter resource limit enforcement"
                            )
    ...
    def _enable_security_context_requirements(
        self, policy_data: Dict[str, Any]
    ) -> List[str]:
    ...
    def _enable_network_policy_enforcement(
        self, policy_data: Dict[str, Any]
    ) -> List[str]:
    ...
    def _apply_cis_customizations(self, policy_data: Dict[str, Any]) -> List[str]:
        ...
        try:
            ...
            policy_data["metadata"]["annotations"][
                "policies.kyverno.io/cis-compliance"
            ] = "true"
    ...
    def _apply_pci_customizations(self, policy_data: Dict[str, Any]) -> List[str]:
        ...
        try:
            ...
            policy_data["metadata"]["annotations"][
                "policies.kyverno.io/pci-compliance"
            ] = "true"
    ...
    def _apply_hipaa_customizations(self, policy_data: Dict[str, Any]) -> List[str]:
        ...
        try:
            ...
            policy_data["metadata"]["annotations"][
                "policies.kyverno.io/hipaa-compliance"
            ] = "true"
