...
class TestCaseGenerator:
    ...
    def generate_comprehensive_test_case(self, policy_content: str, policy_name: str) -> str:
        ...
        try:
            ...
            ai_test = self._generate_ai_test_case(policy_content, policy_data, policy_name)
            ...
            if ai_test:
                ...
                if self._validate_test_case_format(ai_test):
                    ...
                    self.logger.warning(f"AI-generated test case for {policy_name} has invalid format, using template")
    ...
    def generate_test_resources(self, policy_content: str, policy_name: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for resource_type in resource_info["kinds"]:
                ...
                passing_resource = self._generate_passing_resource(resource_type, policy_data, resource_info)
                if passing_resource:
                ...
                failing_resource = self._generate_failing_resource(resource_type, policy_data, resource_info)
                if failing_resource:
                    test_resources.append({
    ...
    def enhance_existing_test_case(self, existing_test: str, policy_content: str) -> str:
...
"""
            ...
            if self._validate_test_case_format(response):
                ...
                self.logger.warning("Enhanced test case has invalid format, returning original")
                return existing_test
    ...
    def _generate_ai_test_case(self, policy_content: str, policy_data: Dict[str, Any], policy_name: str) -> Optional[str]:
        """Use AI to generate comprehensive test case."""
        try:
            # Extract key information about the policy
            rules = policy_data.get("spec", {}).get("rules", [])
            rule_names = [rule.get("name", f"rule-{i+1}") for i, rule in enumerate(rules)]
...
"""
            ...
            try:
                ...
                if documents:
                    ...
                    if self._validate_test_case_format(test_yaml):
                        ...
                        self.logger.warning(f"AI-generated test case for {policy_name} has invalid format")
                        return None
                else:
                    self.logger.warning(f"AI-generated test case for {policy_name} is empty")
                    return None
    ...
    def _validate_test_case_format(self, test_content: str) -> bool:
        ...
        try:
            ...
            if test_data.get("apiVersion") == "cli.kyverno.io/v1alpha1" and test_data.get("kind") == "Test":
    ...
    def _generate_passing_resource(self, resource_type: str, policy_data: Dict[str, Any], 
                                 resource_info: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if resource_info["namespaces"]:
                if "metadata" not in base_resource:
                    base_resource["metadata"] = {}
                base_resource["metadata"]["namespace"] = list(resource_info["namespaces"])[0]
    ...
    def _generate_failing_resource(self, resource_type: str, policy_data: Dict[str, Any], 
                                 resource_info: Dict[str, Any]) -> Optional[Dict[str, Any]]:
