...
class PolicyCatalogManager(PolicyCatalogInterface):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.local_storage = self.catalog_config.get(
        ...
        try:
            ...
            for i, repo_config in enumerate(repos_to_process):
                ...
                if i < len(source_dirs):
                    repo_info[source_dirs[i]] = (
                        repo_url  # Store full HTTPS URL for GitLab/GitHub/etc support
                    )
    ...
    def build_policy_index(self) -> PolicyIndex:
        ...
        try:
            ...
            if not os.path.exists(self.local_storage):
                raise CatalogError(
                    f"Policy catalog directory not found: {self.local_storage}"
                )
            ...
            policy_files = FileUtils.list_files(
                self.local_storage, "*.yaml", recursive=True
            )
            ...
            for policy_file in policy_files:
                try:
                    ...
                    if (
                        filename
                        in ["kyverno-test.yaml", "resource.yaml", "resources.yaml"]
                        or filename.startswith("test-")
                        or filename.endswith("-test.yaml")
                        or ".chainsaw-test" in policy_file
                        or filename.endswith("-bad.yaml")
                        or filename.endswith("-good.yaml")
                    ):
                        continue
                    ...
                    logger.warning(
                        f"Failed to process policy file {policy_file}: {str(e)}"
                    )
                    continue
            ...
            logger.info(
                f"Policy index built with {policy_index.total_policies} policies across {len(policy_index.categories)} categories"
            )
            return policy_index
    ...
    def get_all_policies_lightweight(self) -> List[Dict[str, Any]]:
        ...
        try:
            logger.info(
                "Getting all policies with lightweight metadata for Phase 1 filtering"
            )
            ...
            if not policy_index:
                raise CatalogError(
                    "Policy index not found. Please build the catalog first."
                )
            ...
            logger.info(
                f"Retrieved {len(lightweight_policies)} policies with lightweight metadata"
            )
    ...
    def get_policies_detailed(self, policy_names: List[str]) -> List[Dict[str, Any]]:
        ...
        try:
            logger.info(
                f"Getting detailed information for {len(policy_names)} policies for Phase 2 selection"
            )
            ...
            logger.info(
                f"Retrieved detailed information for {len(detailed_policies)} policies"
            )
    ...
    def _clone_repository(self, url: str, branch: str) -> Optional[str]:
        ...
        try:
            ...
            # Clone repository
            cmd = ["git", "clone", "--depth", "1", "--branch", branch, url, temp_dir]
    ...
    def _process_policy_repositories(
        self, source_dirs: List[str], repo_info: Dict[str, str]
    ) -> None:
        ...
        for policy_dir, rel_policy_dir in policy_dirs:
            try:
                ...
                if os.path.exists(kyverno_test_file):
                    self._copy_kyverno_test_files(
                        source_dir, policy_dir, kyverno_test_dir, dest_policy_dir
                    )
                ...
                # Check for direct kyverno-test.yaml and resource.yaml (fallback case)
                    ...
                    self._copy_direct_test_files(
                        policy_dir, dest_policy_dir, rel_policy_dir
                    )

            except Exception as e:
                logger.error(
                    f"Failed to copy test files for directory {rel_policy_dir}: {str(e)}"
                )
                continue

    def _copy_kyverno_test_files(
        self,
        source_dir: str,
        policy_dir: str,
        kyverno_test_dir: str,
        dest_policy_dir: str,
    ) -> None:
            ...
            try:
                ...
                for resource_ref in all_resource_refs:
                    if isinstance(resource_ref, str) and resource_ref.strip():
                        ...
                        resource_source_path = os.path.join(
                            kyverno_test_dir, resource_ref
                        )
                        ...
                        if os.path.exists(resource_source_path):
                            ...
                            dest_resource_path = os.path.join(
                                dest_policy_dir, clean_resource_path
                            )
                            ...
                            logger.debug(
                                f"Resource file not found: {resource_source_path}"
                            )
    ...
    def _copy_direct_test_files(
        self, policy_dir: str, dest_policy_dir: str, rel_policy_dir: str
    ) -> None:
        ...
        try:
            ...
            if os.path.exists(direct_test_file):
                FileUtils.copy_file(
                    direct_test_file, os.path.join(dest_policy_dir, "kyverno-test.yaml")
                )
                logger.debug(
                    f"Copied direct test file: {os.path.join(rel_policy_dir, 'kyverno-test.yaml')}"
                )
            ...
            if os.path.exists(resource_file):
                FileUtils.copy_file(
                    resource_file, os.path.join(dest_policy_dir, "resource.yaml")
                )
                logger.debug(
                    f"Copied resource file: {os.path.join(rel_policy_dir, 'resource.yaml')}"
                )
            ...
            if os.path.exists(values_file):
                FileUtils.copy_file(
                    values_file, os.path.join(dest_policy_dir, "values.yaml")
                )
                logger.debug(
                    f"Copied values file: {os.path.join(rel_policy_dir, 'values.yaml')}"
                )

        except Exception as e:
            logger.error(
                f"Failed to copy direct test files for {rel_policy_dir}: {str(e)}"
            )
    ...
    def _create_policy_entry(self, policy_file: str) -> Optional[PolicyCatalogEntry]:
        ...
        try:
            ...
            name = metadata.get(
                "name", os.path.basename(policy_file).replace(".yaml", "")
            )
    ...
    def _is_valid_kyverno_policy(self, content: Dict[str, Any]) -> bool:
        ...
        return (
            ...
            and content.get("kind") in ["ClusterPolicy", "Policy"]
            and content.get("apiVersion", "").startswith("kyverno.io/")
            and "spec" in content
            and "rules" in content.get("spec", {})
        )
    ...
    def _find_test_directory(self, policy_file: str) -> Optional[str]:
        ...
        has_test_files = any(
            os.path.exists(os.path.join(policy_dir, test_file))
    ...
    def _extract_tags(self, policy_content: Dict[str, Any], rel_path: str) -> List[str]:
        ...
        tags.extend(
            [part.replace("-", " ").replace("_", " ") for part in path_parts[:-1]]
        )
    ...
    def _load_policy_index(self) -> Optional[PolicyIndex]:
        ...
        try:
            ...
            policy_index.last_updated = datetime.fromisoformat(
                index_data.get("last_updated", datetime.now().isoformat())
            )
