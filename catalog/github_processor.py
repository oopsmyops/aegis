...
class GitHubProcessor:
    ...
    def clone_repository(
        self, url: str, branch: str = "main", depth: int = 1
    ) -> Optional[str]:
        ...
        try:
            ...
            result = subprocess.run(
                cmd, capture_output=True, text=True, timeout=300, cwd=self.temp_dir
            )
    ...
    def find_policy_files(self, repo_dir: str) -> List[str]:
        ...
        try:
            ...
            candidate_files = [
                f.strip() for f in result1.stdout.strip().split("\n") if f.strip()
            ]
            ...
            if result2.returncode != 0:
                logger.warning(
                    f"No policy files with validationFailureAction found in {repo_dir}"
                )
                return []
    ...
    def extract_policies_with_tests(
        self, repo_dir: str, policy_files: List[str]
        ...
        try:
            ...
            for policy_file in policy_files:
                try:
                    ...
                    logger.debug(
                        f"Extracted policy: {rel_path} with {len(test_files)} test files"
                    )
        ...
        try:
            ...
            for rel_path, policy_info in extracted_policies.items():
                try:
                    ...
                    FileUtils.copy_file(
                        policy_info["policy_file"], policy_dest, create_dirs=True
                    )
                    ...
                    for test_file_info in policy_info["test_files"]:
                        ...
                        test_dest = os.path.join(
                            catalog_dir, os.path.dirname(rel_path), test_rel_path
                        )
                        ...
                        if test_file_info["needs_modification"]:
                            ...
                            FileUtils.copy_file(
                                test_source, test_dest, create_dirs=True
                            )
    ...
    def _find_test_files(self, policy_dir: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if os.path.exists(kyverno_test_dir):
                test_files.extend(
                    self._process_kyverno_test_dir(kyverno_test_dir, policy_dir)
                )
    ...
    def _process_kyverno_test_dir(
        self, kyverno_test_dir: str, policy_dir: str
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if os.path.exists(main_test_file):
                ...
                resource_files = self._find_referenced_resources(
                    main_test_file, kyverno_test_dir
                )
                test_files.extend(resource_files)

        except Exception as e:
            logger.warning(
                f"Error processing kyverno test directory {kyverno_test_dir}: {str(e)}"
            )

        return test_files

    def _find_referenced_resources(
        self, test_file: str, test_dir: str
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.warning(
                f"Error finding referenced resources in {test_file}: {str(e)}"
            )
