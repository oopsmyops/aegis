...
class DiscoverCommand(BaseCommand):
    ...
    def execute(
        self,
        output: str = "cluster-discovery.yaml",
...
class QuestionnaireCommand(BaseCommand):
    ...
    def execute(
        self, input_file: str = "cluster-discovery.yaml", batch: bool = False
    ) -> None:
        ...
        try:
            ...
            print(
                f"🚀 You can now run 'aegis recommend' to get policy recommendations."
            )
    ...
    def execute(
        ...
        try:
            ...
            if repos:
                ...
                repo_urls = [
                    repo["url"]
                    for repo in self.config.get("catalog", {}).get("repositories", [])
                ]
...
class RecommendCommand(BaseCommand):
    ...
    def execute(
        self,
        input_file: str = "cluster-discovery.yaml",
        ...
        try:
            ...
            requirements = GovernanceRequirements(
                compliance_frameworks=cluster_data.get(
            ...
            bedrock_client = BedrockClient(
                region=self.config["ai"]["region"], model_id=self.config["ai"]["model"]
            )
            ...
            print(f"📂 Output directory: {output}")
            print(
                f"📊 Total policies recommended: {len(recommendation.recommended_policies)}"
            )
