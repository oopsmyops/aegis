...
def cli(ctx: click.Context, config: Optional[str], verbose: bool, debug: bool):
    ...
    try:
        ...
        click.echo(f"💡 Try running 'aegis config --init' to create default configuration", err=True)
        sys.exit(1)
    
    # Setup logging with enhanced configuration
    log_level = "DEBUG" if (verbose or debug) else ctx.obj['config'].get('logging', {}).get('level', 'INFO')
    ...
    try:
        ...
        if controllers:
            ...
            for controller in controllers[:3]:  # Show first 3
                click.echo(f"     - {controller.get('name', 'Unknown')} ({controller.get('type', 'unknown')})")
        ...
        click.echo(f"💡 Please check the log file for more details: {ctx.obj['config'].get('logging', {}).get('file', './aegis.log')}")
        sys.exit(1)
...
def questionnaire(ctx: click.Context, input: str, batch: bool):
    ...
    try:
        ...
        total_questions = ctx.obj['config'].get('questionnaire', {}).get('total_questions', 20)
        click.echo(f"❓ You will be asked up to {total_questions} yes/no questions about governance requirements")
        click.echo(f"💡 Some questions may have follow-up prompts for additional details")
        ...
        if not requirements.answers:
            ...
            click.echo("💡 You can restart with 'aegis questionnaire' or use 'aegis run --all'")
            sys.exit(1)
        ...
        click.echo(f"   2. Run 'aegis recommend' to get AI-powered policy recommendations")
        click.echo(f"   3. Or run 'aegis run --all --skip-discovery --skip-questionnaire' to continue workflow")
...
def catalog(ctx: click.Context, repos: Optional[str], output: Optional[str], refresh: bool):
    ...
    try:
        ...
        if not repo_urls:
            ...
            click.echo("💡 Example: aegis catalog --repos https://github.com/kyverno/policies")
            sys.exit(1)
        ...
        with click.progressbar(repo_urls, label='Processing repositories') as repos_progress:
        ...
        try:
            ...
            catalog_size = sum(os.path.getsize(os.path.join(dirpath, filename))
                             for dirpath, dirnames, filenames in os.walk(config['catalog']['local_storage'])
        ...
        # Show next steps
        click.echo(f"\n🚀 Next Steps:")
        click.echo(f"   1. Run 'aegis recommend' to get AI-powered policy recommendations")
        click.echo(f"   2. Or run 'aegis run --all --skip-catalog' to continue workflow")
        click.echo(f"   3. Use 'aegis catalog --refresh' to update catalog with latest policies")
...
def recommend(ctx: click.Context, input: str, output: Optional[str], count: Optional[int], 
    ...
    try:
        ...
        if two_phase_config.get('enabled', True):
            ...
            click.echo(f"   • Phase 2 target: {config['ai']['policy_count']['total_target']}")
        else:
        ...
        from utils.progress_utils import show_operation_summary, show_validation_summary, show_file_operations, show_next_steps
        ...
        show_operation_summary("Policy Recommendation", recommendation_stats, duration, True)
        ...
        if validation_results:
            ...
            failed_policy_names = [r.policy_name for r in validation_results if not r.passed]
            show_validation_summary(len(validation_results), passed_count, failed_count, success_rate, failed_policy_names)
        ...
        if fix and validation_results and any(not r.passed for r in validation_results):
            next_steps.insert(1, "Review validation report for any failed policies that need attention")
...
@cli.command()
@click.option('--directory', '-d', required=True, help='Directory containing policies to validate')
...
@click.option('--ai-region', help='AWS region for Bedrock service')
@click.pass_context
def validate(ctx: click.Context, directory: str, fix: bool, ai_model: Optional[str], ai_region: Optional[str]):
    ...
    try:
        ...
        for root, dirs, files in os.walk(directory):
            for file in files:
                if file.endswith('.yaml') and not file.startswith('kyverno-test') and not file.startswith('resource') and not file.startswith('policy-info'):
                    ...
                    try:
                        ...
                        try:
                            ...
                            if not policy_data:
                                ...
                                policy_data = next((doc for doc in documents if doc and doc.get('kind') in ['ClusterPolicy', 'Policy']), None)
                        ...
                        policy_name = policy_data.get('metadata', {}).get('name', file.replace('.yaml', ''))
        ...
        with progress_spinner("Validating policies with Kyverno CLI"):
            validation_results, report_file = validator.validate_policies_with_report(policies, directory)
        ...
        try:
            ...
            click.echo(f"   • Success rate: {(passed/len(validation_results)*100):.1f}%")
        ...
        if not failed_policies and not (test_file_errors if 'test_file_errors' in locals() else []):
            # Check if any policies failed at the policy level (fallback)
            failed_count = len(validation_results) - sum(1 for r in validation_results if r.passed)
            if failed_count > 0:
                click.echo(f"\n❌ Failed Policies:")
                for result in validation_results:
                    if not result.passed:
                        ...
                        if len(result.errors) > 2:
                            click.echo(f"     - ... and {len(result.errors) - 2} more errors")
        ...
        failed_tests = validation_report.get('failed_tests', 0) if validation_report else 0
        if failed_tests > 0:
            sys.exit(1)
...
@click.option('--skip-discovery', is_flag=True, help='Skip cluster discovery (use existing cluster-discovery.yaml)')
@click.option('--skip-questionnaire', is_flag=True, help='Skip questionnaire (use existing requirements in cluster-discovery.yaml)')
@click.option('--skip-catalog', is_flag=True, help='Skip catalog creation (use existing catalog)')
@click.option('--output', '-o', help='Output directory for recommended policies')
@click.option('--count', type=int, help='Target number of policies to recommend')
@click.option('--fix', is_flag=True, help='Enable Kyverno validation and automatic fixing')
@click.pass_context
def run(ctx: click.Context, run_all: bool, skip_discovery: bool, skip_questionnaire: bool, 
        skip_catalog: bool, output: Optional[str], count: Optional[int], fix: bool):
    ...
    if run_all:
        ...
        try:
            ...
            click.echo(f"⚙️  Configuration: {ctx.obj.get('config_manager', {}).config_path or 'default'}")
            ...
            if not skip_discovery:
                ...
                try:
                    ...
                    click.echo(f"💡 Use --skip-discovery to skip this step if you have existing cluster-discovery.yaml")
                    sys.exit(1)
            else:
                show_progress("Cluster Discovery (Skipped)", 1)
                if not os.path.exists("cluster-discovery.yaml"):
                    click.echo(f"❌ cluster-discovery.yaml not found. Cannot skip discovery.")
                    sys.exit(1)
            ...
            if not skip_questionnaire:
                ...
                try:
                    ...
                    click.echo(f"💡 Use --skip-questionnaire to skip this step if requirements are already in cluster-discovery.yaml")
                    sys.exit(1)
            else:
                show_progress("Requirements Questionnaire (Skipped)", 2)
                click.echo(f"⏭️  Using existing requirements from cluster-discovery.yaml")
            ...
            if not skip_catalog:
                ...
                try:
                    ...
                    click.echo(f"💡 Use --skip-catalog to skip this step if you have an existing catalog")
                    sys.exit(1)
            else:
                ...
                index_path = config['catalog']['index_file']
                if not os.path.exists(index_path):
                    click.echo(f"❌ Policy index not found: {index_path}. Cannot skip catalog creation.")
                    sys.exit(1)
            ...
            click.echo(f"📂 Output directory: {output or ctx.obj['config']['output']['directory']}")
...
def health(ctx: click.Context):
    ...
    try:
        ...
        click.echo(f"   • Config file: {ctx.obj.get('config_manager', {}).config_path or 'default'}")
    except Exception as e:
    ...
    try:
        ...
        if ai_config.get('provider') == 'aws-bedrock':
            ...
            bedrock_client = BedrockClient(
                ...
                model_id=ai_config.get('model', 'anthropic.claude-3-sonnet-20240229-v1:0')
            )
            ...
            click.echo(f"ℹ️  AWS Bedrock: Not configured (using alternative AI provider)")
    except Exception as e:
    ...
    try:
        import subprocess
        result = subprocess.run(['kyverno', 'version'], capture_output=True, text=True, timeout=10)
    ...
    if not os.path.exists(config.get('catalog', {}).get('index_file', './policy-index.json')):
...
def main():
    ...
    try:
        ...
        if os.environ.get('AEGIS_DEBUG') or '--debug' in sys.argv:
            ...
            click.echo(f"💡 Run with --debug flag for detailed error information", err=True)
