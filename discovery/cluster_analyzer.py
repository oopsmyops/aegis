...
class ClusterAnalyzer(LoggerMixin):
        ...
        try:
            ...
            for node in nodes.items:
                ...
                if any(key.startswith("eks.amazonaws.com") for key in labels.keys()):
                    return "eks"
                if "node.kubernetes.io/instance-type" in labels and labels.get(
                    "kubernetes.io/hostname", ""
                ).endswith(".compute.internal"):
                    return "eks"
    ...
    def scan_third_party_controllers(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for controller in crd_controllers:
                ...
                has_workload = any(
                    wc["type"] == controller["type"]
                    and (
                        wc["namespace"] == controller["namespace"]
                        or controller["namespace"] == "cluster-wide"
                    )
    ...
    def _analyze_workload(
        self, name: str, namespace: str, containers: List, kind: str
    ...
    def _is_system_controller(self, name: str, namespace: str) -> bool:
        ...
        if namespace in system_namespaces and any(
            sys_ctrl in name for sys_ctrl in system_controllers
        ):
            return True
    ...
    def analyze_security_features(self) -> Dict[str, bool]:
        ...
        try:
            ...
            security_features["network_policies_supported"] = (
                self._check_network_policies()
            )

            # Check for security-related admission controllers
            security_features["admission_controllers"] = (
                self._detect_admission_controllers()
            )

            # Check for pod security contexts
            security_features["security_contexts_enforced"] = (
                self._check_security_contexts()
            )
