...
class QuestionnaireInterface(ABC):
    ...
    @abstractmethod
    def append_to_cluster_yaml(self, requirements: GovernanceRequirements, yaml_path: str) -> None:
...
class AIPolicySelectorInterface(ABC):
    ...
    @abstractmethod
    def select_policies(self, cluster_info: ClusterInfo, requirements: GovernanceRequirements) -> List[PolicyCatalogEntry]:
        """Select appropriate policies using AI."""
        pass
    
    @abstractmethod
    def determine_categories(self, cluster_info: ClusterInfo, selected_policies: List[PolicyCatalogEntry]) -> List[str]:
