...
def test_error_handling():
    ...
    error_commands = [
        (["python", "main.py", "discover", "--context", "nonexistent"], "Invalid context"),
        (["python", "main.py", "questionnaire", "--input", "nonexistent.yaml"], "Missing input file"),
        (["python", "main.py", "recommend", "--input", "nonexistent.yaml"], "Missing cluster data"),
        (["python", "main.py", "validate", "--directory", "nonexistent"], "Missing directory"),
    ]
    ...
    for cmd, description in error_commands:
        ...
        try:
            ...
            if result.returncode == 1:  # Expected failure
                ...
                print(f"⚠️  Command failed with unexpected exit code: {result.returncode}")
