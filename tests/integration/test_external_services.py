...
class TestExternalServiceIntegration(unittest.TestCase):
    ...
    def test_github_repository_access(self):
        ...
        try:
            ...
            cloned_path = processor.clone_repository(
                valid_url, depth=1
            )  # Shallow clone
            ...
            if cloned_path:
                ...
                print(
                    f"✅ GitHub access successful. Found {len(policy_files)} policy files"
                )
            else:
    ...
    def test_kubernetes_cluster_access(self):
        ...
        try:
            ...
            print(
                f"✅ Kubernetes access successful. Version: {cluster_info.get('kubernetes_version')}"
            )
...
@pytest.mark.skipif(
    not SKIP_EXTERNAL_TESTS, reason="Mock tests only when external tests are disabled"
)
class TestMockedExternalServices(unittest.TestCase):
    ...
    def test_kyverno_validator_with_mock(self, mock_subprocess):
        ...
        mock_subprocess.return_value = Mock(
            returncode=0, stdout="kyverno version v1.10.0"
        )
...
class TestServiceErrorHandling(unittest.TestCase):
    ...
    def test_bedrock_connection_failure(self):
        ...
        config = {
            "ai": {
                ...
                "max_tokens": 1000,
                "temperature": 0.1,
                "error_handling": {"enable_fallbacks": False, "max_retry_attempts": 1},
            }
        }
    ...
    def test_github_access_failure(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            processor = GitHubProcessor(temp_dir)

        with patch("catalog.github_processor.subprocess.run") as mock_subprocess:
            mock_subprocess.return_value = Mock(
                returncode=1, stderr="Repository not found"
            )
