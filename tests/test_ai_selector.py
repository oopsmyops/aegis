...
from models import (
    ...
    RequirementAnswer, ThirdPartyController, ControllerType
)
...
class TestAIPolicySelector(unittest.TestCase):
    ...
    def test_extract_lightweight_policies_from_index(self):
        ...
        lightweight_policies = self.ai_selector._extract_lightweight_policies_from_index(self.policy_index)
        ...
        self.assertLessEqual(len(first_policy['tags']), 5)  # Tags should be limited to 5
    ...
    @patch('ai.ai_policy_selector.AIPolicySelector._extract_lightweight_policies_from_index')
    def test_phase_one_filter_success(self, mock_extract):
        ...
        self.mock_bedrock_client.send_request_with_fallback.return_value = '["policy-1", "policy-2"]'
    ...
    def test_prepare_phase_two_context(self):
        ...
        self.assertEqual(context['candidate_policies'][0]['name'], 'require-pod-resources')
        self.assertEqual(context['candidate_policies'][1]['name'], 'restrict-image-registries')
    ...
    def test_parse_phase_two_response_valid_json(self):
        ...
        self.assertIn('require-pod-resources', self.ai_selector._phase_two_customizations)
        self.assertIn('restrict-image-registries', self.ai_selector._phase_two_customizations)
    ...
    def test_map_detailed_policies_to_entries(self):
        ...
        entries = self.ai_selector._map_detailed_policies_to_entries(policy_names, detailed_policies)
        ...
        self.assertEqual(entry.relative_path, 'best-practices/require-pod-resources.yaml')
    ...
    def test_apply_comprehensive_customization(self):
        ...
        customized = self.ai_selector._apply_comprehensive_customization(policies, self.requirements)
        
        self.assertEqual(len(customized), 1)
        customized_policy = customized[0]
        
        # Check registry customization was applied
        registry_tags = [tag for tag in customized_policy.tags if tag.startswith('registry:')]
        self.assertGreater(len(registry_tags), 0)
        
        # Check AI customization was applied
        ai_custom_tags = [tag for tag in customized_policy.tags if tag.startswith('ai_custom:')]
    ...
    def test_phase_two_select_success(self, mock_catalog_manager_class):
        ...
        self.mock_bedrock_client.send_request_with_fallback.return_value = bedrock_response
        ...
        mock_catalog_manager.get_policies_detailed.assert_called_once_with(candidate_names)
        # Verify AI request was made (could be either send_request or send_request_with_fallback)
        assert (self.mock_bedrock_client.send_request.called or 
                self.mock_bedrock_client.send_request_with_fallback.called)
