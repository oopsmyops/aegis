...
class TestClusterDiscovery:
    ...
    def test_initialize_kubernetes_client_success(self, mock_api_client, mock_load_config):
    ...
    def test_initialize_kubernetes_client_failure(self, mock_incluster_config, mock_load_config):
        """Test Kubernetes client initialization failure."""
        from kubernetes.config.config_exception import ConfigException
        
        # Mock in-cluster config to fail first with ConfigException
        mock_incluster_config.side_effect = ConfigException("Service host/port is not set.")
    ...
    def test_discover_cluster_success(self, mock_security, mock_resources, 
                                    mock_controllers, mock_managed, mock_basic, mock_init):
    ...
    def test_export_to_yaml_failure(self, mock_open, mock_makedirs):
        ...
        with pytest.raises(ClusterDiscoveryError):
            self.discovery.export_to_yaml(test_data, '/tmp/test.yaml')
...
class TestClusterAnalyzer:
    ...
    def test_scan_third_party_controllers(self, mock_crs, mock_sts, mock_ds, mock_deploy):
        """Test third-party controller scanning."""
        # Mock return values
        mock_deploy.return_value = [
            {'name': 'nginx-ingress', 'namespace': 'ingress', 'type': 'ingress', 'kind': 'deployment'}
        ]
        mock_ds.return_value = [
            {'name': 'fluentd', 'namespace': 'logging', 'type': 'monitoring', 'kind': 'daemonset'}
        ]
        mock_sts.return_value = []
        mock_crs.return_value = [
            {'name': 'certificates.cert-manager.io', 'namespace': 'cluster-wide', 'type': 'secrets', 'kind': 'custom-resource-definition'}
        ]
        ...
        assert any(controller['name'] == 'certificates.cert-manager.io' for controller in result)
    
    def test_classify_controller_type(self):
        """Test controller type classification."""
        # Test known patterns
        assert self.analyzer._classify_controller_type('nginx-ingress-controller') == 'ingress'
        ...
        assert self.analyzer._classify_controller_type('prometheus-operator') == 'monitoring'
        assert self.analyzer._classify_controller_type('kyverno-controller') == 'security'
        
        # Test unknown pattern
        assert self.analyzer._classify_controller_type('unknown-controller') == 'unknown'
