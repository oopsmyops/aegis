...
class ProgressTracker:
    ...
    def complete_step(
        ...
        if self.step_times:
            ...
            click.echo(
                f"{status} Step {self.current_step} completed in {step_duration:.1f}s"
            )
            if message:
...
def show_operation_summary(
    operation: str, stats: Dict[str, Any], duration: float, success: bool = True
) -> None:
